...
import { Search, Plus, Loader2, Users, PhoneIncoming, PhoneOutgoing, PhoneMissed, Phone, Video, List, LayoutGrid } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={`w-full md:w-80 lg:w-[360px] border-r border-gray-200 dark:border-[#272729] flex flex-col bg-gray-50 dark:bg-[#0a0a0a] shrink-0 z-50 transition-transform duration-300 ${selectedRoomId ? 'hidden md:flex' : 'flex absolute md:relative inset-y-0 left-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col min-h-0 bg-white dark:bg-[#0a0a0a] transition-colors">
           {isLoadingSidebar ? (
               <div className="flex justify-center py-10"><Loader2 className="w-8 h-8 text-blue-500 animate-spin" strokeWidth={3} /></div>
           ) : activeTab === 'chats' ? (
             <div className="flex flex-col pb-4">
               <div className="p-3 border-b border-gray-200 dark:border-[#272729] bg-gray-50 dark:bg-[#0f0f0f] transition-all duration-300">
                  <div className="flex items-center justify-between mb-2">
                      <h3 className="text-[10px] font-extrabold text-gray-500 uppercase tracking-widest ml-1">Your Squads</h3>
                      <div className="flex items-center gap-1.5">
                          <button onClick={() => setIsGroupsExpanded(!isGroupsExpanded)} className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors shadow-sm border ${isGroupsExpanded ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 border-blue-200 dark:border-blue-500/30' : 'bg-white dark:bg-white/5 text-gray-500 hover:bg-gray-100 dark:hover:bg-white/10 border-gray-200 dark:border-transparent'}`} title={isGroupsExpanded ? "Collapse to Row" : "Expand to List"}>
                              {isGroupsExpanded ? <LayoutGrid size={14} strokeWidth={2.5} /> : <List size={14} strokeWidth={2.5} />}
                          </button>
                          <button onClick={() => setShowGroupModal(true)} className="w-7 h-7 rounded-full bg-blue-600 hover:bg-blue-500 flex items-center justify-center text-white transition-colors shadow-sm">
                      ...
                      isGroupsExpanded ? (
                          <div className="space-y-1 mt-3">
                              {groupRooms.map((room: any) => (
                                  <div key={room.room_id} onClick={() => setSelectedRoomId(room.room_id)} className={`flex items-center gap-3 px-3 py-2.5 rounded-2xl cursor-pointer transition-all border ${selectedRoomId === room.room_id ? 'bg-blue-50 dark:bg-white/10 border-blue-200 dark:border-white/10 shadow-sm' : 'bg-white dark:bg-transparent hover:bg-gray-100 dark:hover:bg-white/5 border-transparent'}`}>
                                      <div className="relative shrink-0">
                                          <div className={`w-10 h-10 rounded-[1rem] flex items-center justify-center text-sm font-extrabold text-white overflow-hidden border ${selectedRoomId === room.room_id ? 'bg-blue-600 border-blue-700 shadow-[inset_0_2px_4px_rgba(255,255,255,0.4)]' : 'bg-blue-600 border-blue-700'}`}>
                                          ...
                                          {room.unread_count > 0 && <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-blue-500 rounded-full border-2 border-white dark:border-[#0f0f0f] shadow-sm animate-pulse"></div>}
                                      </div>
                                      <div className="flex-1 min-w-0">
                                          <div className="flex justify-between items-center mb-0.5">
                                              <span className={`text-[14px] truncate ${selectedRoomId === room.room_id ? 'text-blue-700 dark:text-blue-400 font-extrabold' : room.unread_count > 0 ? 'text-gray-900 dark:text-white font-extrabold' : 'text-gray-700 dark:text-gray-200 font-bold'}`}>{room.name || 'Group'}</span>
                          ...
                          <div className="flex gap-2 overflow-x-auto scrollbar-hide snap-x pb-2 pt-1">
                             {groupRooms.map((room: any) => (
                               <div key={room.room_id} onClick={() => setSelectedRoomId(room.room_id)} className={`snap-start shrink-0 w-[72px] h-[76px] rounded-2xl flex flex-col items-center justify-center p-1.5 cursor-pointer transition-all border-2 relative ${selectedRoomId === room.room_id ? 'bg-blue-50 dark:bg-blue-600/20 border-blue-500 dark:border-blue-500 shadow-[inset_0_2px_4px_rgba(255,255,255,0.7)] dark:shadow-[inset_0_2px_4px_rgba(255,255,255,0.1)]' : 'bg-white dark:bg-[#1a1a1a] border-gray-200 dark:border-[#272729] hover:border-blue-300 dark:hover:border-gray-500 shadow-sm'}`}>
                                  
                                  <div className="w-[34px] h-[34px] rounded-xl bg-blue-600 flex items-center justify-center text-sm font-bold text-white mb-1.5 shadow-[inset_0_2px_4px_rgba(255,255,255,0.4)] overflow-hidden border border-blue-700">
                                    {room.avatarUrl || room.avatar_url ? (
                                        <img src={room.avatarUrl || room.avatar_url} alt={room.name} className="w-full h-full object-cover" />
                                    ) : (
                                        (room.name || 'G').charAt(0).toUpperCase()
                                  ...
                                  {room.unread_count > 0 && <span className="absolute top-1.5 right-1.5 w-3 h-3 bg-blue-500 rounded-full border-2 border-white dark:border-[#1a1a1a] shadow-[0_0_8px_rgba(59,130,246,0.6)] animate-pulse"></span>}
                               </div>
                             ))}
                           </div>
                      )
                  ) : (
                    <div className="text-center text-[11px] font-bold text-gray-500 dark:text-gray-600 py-3 bg-white dark:bg-[#1a1a1a] border-2 border-dashed border-gray-200 dark:border-[#272729] rounded-2xl cursor-pointer hover:bg-gray-100 dark:hover:bg-white/5 transition-colors mt-2" onClick={() => setShowGroupModal(true)}>
                    ...
                    {callHistory?.length > 0 ? callHistory.map((call: any, idx: number) => {
                        ...
                        if (isOutgoing) {
                            ...
                            if (knownUser) {
                                ...
                                peerName = call.receiverName || call.peerName || 'Unknown User';
                                peerAvatar = call.receiverAvatar || call.peerAvatar;
                            }
                        }
                            ...
                            <div className="space-y-2">
                               {messageRequests.map((req: any) => (
                                <div key={req.room_id} onClick={() => setSelectedRoomId(req.room_id)} className={`rounded-2xl p-3 border cursor-pointer transition-all shadow-sm ${selectedRoomId === req.room_id ? 'bg-blue-50 dark:bg-white/10 border-blue-200 dark:border-white/10' : 'bg-gray-50 dark:bg-[#1a1a1a] border-gray-200 dark:border-[#272729] hover:border-gray-300 dark:hover:bg-white/5'}`}>
                                       ...
                                       <p className="text-[11px] text-gray-600 dark:text-gray-400 font-medium italic bg-white dark:bg-black/20 border border-gray-200 dark:border-transparent p-2.5 rounded-xl line-clamp-2 shadow-inner">"{renderCleanPreview(req.last_message_preview)}"</p>
                                    </div>
                               ))}
                            </div>
                          </>
